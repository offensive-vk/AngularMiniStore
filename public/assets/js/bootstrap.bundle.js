...
(function (global, factory) {
  ...
  const getSelector = (element) => {
    ...
    return selector
      ? selector.split(",").map((sel) => parseSelector(sel)).join(",")
      : null;
  };
  ...
  EventHandler.on(
    ...
    function (event) {
      ...
      for (
        const element of SelectorEngine.getMultipleElementsFromSelector(this)
      ) {
        Collapse.getOrCreateInstance(element, {
          toggle: false,
        }).toggle();
  ...
  */
  ...
  class Tab extends BaseComponent {
    ...
    _keydown(event) {
      ...
      event.preventDefault();
      const children = this._getChildren().filter((element) =>
        !isDisabled(element)
      );
